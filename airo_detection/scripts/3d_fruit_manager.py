...
class LidarReprojector:
    def __init__(self):
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer([self.sub_lidar, self.sub_image, self.sub_2dfruit, self.sub_odom], 10, 0.1)
    ...
    def callback(self, lidar_msg, image_msg, two_d_fruit_keypoints_msg, odom_msg):
        ...
        colored_image_points = self.colorize_uvd_visualization(uvd_points, image)  #colored_image_points only for visualization, could delete
        ...
        if(fruit_depth > 3):
            ...
            self.fruit_database.add_fruit_marker('yellow', curr_yellow_id, XYZ_yellow, abs(1))
    ...
    def lidar_msg_to_xyz_array(self, lidar_msg):
        ...
        points_arr = points_arr[points_arr[:, 0] >= 0]
    ...
    def find_fruit_depth(self, uvd_points, fruit_point):
        ...
        if selected_points.size == 0:
            ...
            print("Minimum distance point: ", min_point)
            return fruit_depth
